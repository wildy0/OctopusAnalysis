...
if __name__ == '__main__':
    ...
    filename = filedialog.askopenfilename(title="Select Octopus csv file export, or xls(s) version",
    ...
    remove_set = 24*60  # remove anything less than 24 hours for a day of data
    missing_data_days = np.sum(durations.to_numpy() < remove_set)  # look for days with less than 24-hours of data
    ...
    if missing_data_days > 0:
        ...
        if not nodelete:
            ...
            big_data_removed = big_data[durations_t >= remove_set]  # only keep days with 24-hours of data in them
            if debug:
                big_data_missing = big_data[durations_t < remove_set]  # only keep days with 24-hours of data in them
                d_file = open(os.path.join(filepath, 'missing_data' + ".txt"), "w")
                d_file.write(big_data_missing.to_string())
                d_file.close()
        else:
